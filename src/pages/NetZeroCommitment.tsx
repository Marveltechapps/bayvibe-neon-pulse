  ...
  return <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section className="relative py-20 bg-gradient-to-br from-primary/10 to-secondary/10">
          ...
          <img src="/lovable-uploads/44c183d2-beca-48cf-abd8-95e23fd702bf.png" alt="Net Zero Sustainability Icons" className="absolute inset-0 w-full h-full object-cover opacity-40 z-0" />
        ...
        <section className="py-16 bg-card/30">
          <div className="section-container">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {milestones.map((milestone, index) => <Card key={milestone.year} className="relative overflow-hidden group hover:shadow-lg transition-all duration-300">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-2xl font-bold gradient-text">{milestone.year}</div>
                      <div className={`w-3 h-3 rounded-full ${milestone.status === 'completed' ? 'bg-primary' : milestone.status === 'in-progress' ? 'bg-yellow-500' : 'bg-muted'}`}></div>
                    </div>
                  ...
                  <div className={`absolute bottom-0 left-0 right-0 h-1 ${milestone.status === 'completed' ? 'bg-primary' : milestone.status === 'in-progress' ? 'bg-yellow-500' : 'bg-muted'}`}></div>
                </Card>)}
            </div>
          </div>
        </section>
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div>
                ...
                <div className="space-y-6">
                  ...
                  return <div key={commitment.title} className="flex gap-4 p-4 rounded-lg bg-card/30 hover:bg-card/50 transition-colors duration-300">
